    ...
    for row1 in rowers:
        for row2 in rowers:
            if row1 == row2:
                ...
                combined_variance = sigma_A**2 + sigma_B**2
                
                # Handle cases where combined variance is zero or very small
                if combined_variance < 1e-10:  # Use a small threshold instead of exact zero
                    if mu_A < mu_B:
                        prob_matrix.loc[row1, row2] = "100%"
                    elif mu_A > mu_B:
                        prob_matrix.loc[row1, row2] = "0%"
                    else:
                        prob_matrix.loc[row1, row2] = "50%"
                else:
                    # Normal case - compute probability using normal CDF
                    z_score = (mu_B - mu_A) / np.sqrt(combined_variance)
...
def generate_likelihood(row, x_vals):
    ...
    if std_dev < 1e-10:
        # Return impulse function - all zeros except at the mean
        return [1.0 if abs(x - mean) < 1e-10 else 0.0 for x in x_vals] # PDF or CDF
        
    return scipy.stats.norm.pdf(x_vals, mean, std_dev).tolist() # probability density function
    # return scipy.stats.norm.cdf(x_vals, mean, std_dev).tolist() # cumulative
...
def generate_side_chart(col1, side_df):
    try:
        ...
        # After generating all likelihoods, get y_max
        all_likelihood_values = []
        for likelihood_list in side_df["Likelihood Chart"]:
            all_likelihood_values.extend(likelihood_list)

        # Find the maximum value across all charts
        max_likelihood = max(all_likelihood_values) #if all_likelihood_values else 0.25

        # Add a scaling factor to reduce dead space (e.g., 1.1 for 10% headroom)
        scaling_factor = 1.05
        y_max_value = max_likelihood * scaling_factor
    
        # Define column configuration
        column_config = {
            "Likelihood Chart": st.column_config.BarChartColumn(
                label="Confidence",
                y_min=0,
            ...
            "max_correlation": st.column_config.NumberColumn(
                ...
                format="%.2f",
                help="Max correlation with other athletes"
            ),
            "max_correlated": st.column_config.TextColumn(
                label="Highest Correlated Athlete(s)",
                help="Athlete with the highest correlation"
            ),
            "min_correlation": st.column_config.NumberColumn(
                label="Lowest Correlation",
                format="%.2f",
                help="Min correlation with other athletes"
            ),
            "min_correlated": st.column_config.TextColumn(
                label="Loewst Correlated Athlete(s)",
                help="Athlete with the lowest correlation"
            ),
        }

        num_rows = len(side_df)
        calculated_height = (num_rows + 1) * 35 + 3

        # Remove nans from the likelihood chart
        side_df['Likelihood Chart'] = side_df['Likelihood Chart'].apply(
            lambda lst: [0.0 if isinstance(x, float) and math.isnan(x) else x for x in lst]
        )
        col1.dataframe(
        ...
        st.error(f"Error generating side chart: {e}")
...
def generate_confidence_bars_with_gradient_working(side_df, confidence=50):
    ...
    for index, row in side_df.iterrows():
        ...
        # Check if std_dev is zero or very small
        if std_dev < 1e-10:
            lower_percentile = mean
            upper_percentile = mean
        else:
            lower_percentile = scipy.stats.norm.ppf(0.5 - ((confidence/2.0) / 100.0), mean, std_dev)
            upper_percentile = scipy.stats.norm.ppf(0.5 + ((confidence/2.0) / 100.0), mean, std_dev)
