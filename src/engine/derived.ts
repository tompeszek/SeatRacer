...
/**
 * Rowers who always rowed together, taken as one unit. The unit's average
 * coefficient is often estimable when the members' own are not.
 */
export interface BlockStat {
  members: string[]
  /** Average coefficient of the members. */
  coefficient: number
  lower: number
  upper: number
  /** Seconds per 500m behind the fastest comparable unit; NaN if none. */
  behind: number
  races: number
  comparable: boolean
  /**
   * Per-seat gap from an average rower: each member against the average
   * comparable rower on their own side, averaged over the members. NaN
   * when the data cannot pin it down.
   */
  vsAverage: number
  vsLower: number
  vsUpper: number
...
/** Contrast vector: c'b is the block's per-seat gap from an average rower. */
function vsAverageContrast(design: Design, members: number[], groupOf: number[]): Float64Array | null {
  const { athletes } = design
  const c = new Float64Array(design.columns.length)
  for (const m of members) {
    const side = athletes[m].slice(-1)
    const peers = athletes
      .map((_, i) => i)
      .filter((i) => athletes[i].slice(-1) === side && groupOf.filter((g) => g === groupOf[i]).length > 1)
    if (peers.length === 0) return null
    c[m] += 1 / members.length
    for (const i of peers) c[i] -= 1 / (members.length * peers.length)
  }
  return c
}

export function blockStats(design: Design, fit: FitResult): BlockStat[] {
  const { athletes, x } = design
  const n = x.length
  const k = design.columns.length
  // Two athletes are one unit when their columns are proportional: they
  // never appeared apart.
  const col = (c: number) => Float64Array.from({ length: n }, (_, r) => x[r][c])
  const cols = athletes.map((_, c) => col(c))
  const together = (a: number, b: number) => {
    let ab = 0
    let aa = 0
    let bb = 0
    for (let r = 0; r < n; r++) {
      ab += cols[a][r] * cols[b][r]
      aa += cols[a][r] ** 2
      bb += cols[b][r] ** 2
    }
    return aa > 0 && bb > 0 && ab * ab > (1 - 1e-9) * aa * bb
  }
  const parent = athletes.map((_, i) => i)
  const find = (i: number): number => (parent[i] === i ? i : (parent[i] = find(parent[i])))
  for (let i = 0; i < athletes.length; i++) {
    for (let j = i + 1; j < athletes.length; j++) if (together(i, j)) parent[find(i)] = find(j)
  }
  ...
  const blocks = [...byRoot.values()].filter((m) => m.length > 1)
  if (blocks.length === 0) return []

  const indicator = (members: number[]) => {
    const c = new Float64Array(k)
    for (const i of members) c[i] = 1 / members.length
    return c
  }
  const ind = blocks.map(indicator)
  const estimable = estimableTest(design)
  const bp = blocks.map((_, i) => i)
  const bfind = (i: number): number => (bp[i] === i ? i : (bp[i] = bfind(bp[i])))
  for (let a = 0; a < blocks.length; a++) {
    for (let b = a + 1; b < blocks.length; b++) {
      const c = ind[a].map((v, i) => v - ind[b][i])
      if (estimable(c)) bp[bfind(a)] = bfind(b)
    }
  }
  ...
  const stats: BlockStat[] = blocks.map((members, i) => {
    const vs = vsAverageContrast(design, members, groupOf)
    const vsOk = vs != null && estimable(vs)
    const vsValue = vsOk ? vs!.reduce((acc, v, col) => acc + v * fit.params[col], 0) : NaN
    const vsHalf = vsOk ? halfWidth(vs!) : NaN
    return {
      members: members.map((m) => athletes[m]),
      coefficient: members.reduce((acc, m) => acc + fit.params[m], 0) / members.length,
      lower: -Infinity,
      upper: Infinity,
      behind: NaN,
      races: x.filter((row) => row[members[0]] !== 0).length,
      comparable: blocks.some((_, j) => j !== i && bfind(j) === bfind(i)),
      vsAverage: vsValue,
      vsLower: vsValue - vsHalf,
      vsUpper: vsValue + vsHalf,
    }
  })
  const groups = new Set(blocks.map((_, i) => bfind(i)))
  for (const g of groups) {
    const idx = blocks.map((_, i) => i).filter((i) => bfind(i) === g)
    if (idx.length < 2) continue
    const fastest = Math.min(...idx.map((i) => stats[i].coefficient))
    for (const i of idx) {
      stats[i].behind = stats[i].coefficient - fastest
      // Interval on the unit's gap from the group average.
      const c = ind[i].map((v, col) => v - idx.reduce((acc, j) => acc + ind[j][col], 0) / idx.length)
      const half = halfWidth(c)
      if (!Number.isFinite(half)) continue
      stats[i].lower = stats[i].coefficient - half
      stats[i].upper = stats[i].coefficient + half
    }
  }
  return stats
}
